...
<head>
    ...
    <title>تتبع طلبيتك مباشرة | MED EXPRESS</title>
...
<body class="antialiased selection:bg-emerald-500 selection:text-white pb-12">
    ...
    <div class="bg-emerald-900 text-white text-[11px] font-bold py-2.5 px-4 sticky top-0 z-50 shadow-sm">
        <div class="max-w-6xl mx-auto flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span>بوابة التتبع المباشر لطلبيات متجر MED EXPRESS</span>
            </div>
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 py-12 space-y-8">
        ...
        <div class="track-card p-6 sm:p-10 space-y-6 text-center">
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-slate-900">تتبع مسار شحنتك المباشر</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-1.5">أدخل رقم الهاتف الذي قمت بالطلب به أو كود التتبع لمعرفة حالة طردك</p>
            </div>
        ...
        <!-- 📦 Order Result View (Displayed if order is found) -->
        @if(isset($order) && $order)
            @php
                ...
                $whatsappChangeMsg = "السلام عليكم، أنا الزبون " . $order->customer_name . " بخصوص طلبيتي رقم (" . $order->tracking_number . ")، أريد تعديل وقت أو عنوان التوصيل وشكراً.";
            @endphp
            ...
            <div class="track-card p-6 sm:p-8 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-slate-100 pb-4">
                    <div>
                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">معلومات الطلبية</span>
                    ...
                    ">
                        @if($order->status == 'nouveau') ⚡ قيد المعالجة المبدئية @endif
                        @if($order->status == 'confirme') 📞 تم التأكيد وجاري التجهيز @endif
                        @if($order->status == 'en_livraison') 🚚 الطرد مع الموزع وفي الطريق إليك @endif
                        @if($order->status == 'livre') ✓ تم الاستلام بنجاح @endif
                        @if($order->status == 'annule') ❌ ملغاة @endif
                    </span>
                </div>

                <!-- 🛤️ Visual Progress Stepper -->
                <div class="space-y-4 text-xs font-bold relative pr-6 before:content-[''] before:absolute before:right-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
                    
                    <!-- Step 1 -->
                    <div class="relative flex items-start gap-3">
                        <span class="absolute -right-6 top-0.5 w-4 h-4 rounded-full bg-emerald-500 border-2 border-white flex items-center justify-center text-[9px] text-white">✓</span>
                        <div>
                            <div class="text-slate-900">تم تسجيل طلبك بالمتجر بنجاح</div>
                    ...
                    <!-- Step 2 -->
                    <div class="relative flex items-start gap-3">
                        <span class="absolute -right-6 top-0.5 w-4 h-4 rounded-full {{ in_array($order->status, ['confirme', 'en_livraison', 'livre']) ? 'bg-blue-500 text-white' : 'bg-slate-200 text-slate-400' }} border-2 border-white flex items-center justify-center text-[9px]">
                            {{ in_array($order->status, ['confirme', 'en_livraison', 'livre']) ? '✓' : '2' }}
                        </span>
                        <div>
                            <div class="text-slate-900">تم تأكيد الطلب وتغليف الطرد للشحن</div>
                            <span class="text-[10px] text-slate-400 font-normal">المستودع اللوجستي المركزي</span>
                        </div>
                    </div>

                    <!-- Step 3 -->
                    <div class="relative flex items-start gap-3">
                        <span class="absolute -right-6 top-0.5 w-4 h-4 rounded-full {{ in_array($order->status, ['en_livraison', 'livre']) ? 'bg-purple-500 text-white' : 'bg-slate-200 text-slate-400' }} border-2 border-white flex items-center justify-center text-[9px]">
                            {{ in_array($order->status, ['en_livraison', 'livre']) ? '✓' : '3' }}
                        </span>
                        <div>
                            <div class="text-slate-900">الشحنة في طريق التوصيل مع موزع مدينتك</div>
                            <span class="text-[10px] text-emerald-600 font-bold">📍 الوجهة: {{ $order->city }}</span>
                        </div>
                    </div>

                    <!-- Step 4 -->
                    <div class="relative flex items-start gap-3">
                        <span class="absolute -right-6 top-0.5 w-4 h-4 rounded-full {{ $order->status == 'livre' ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400' }} border-2 border-white flex items-center justify-center text-[9px]">
                            {{ $order->status == 'livre' ? '✓' : '4' }}
                        </span>
                        <div>
                            <div class="text-slate-900">تسليم الطرد وفحص السلعة والدفع نقداً</div>
                            <span class="text-[10px] text-slate-400 font-normal">الدفع عند الاستلام: {{ $order->total_amount }} DH</span>
                        </div>
                    </div>
                ...
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2 text-xs">
                    <h3 class="font-black text-slate-900">محتوى الطلبية:</h3>
                ...
                <!-- 💬 Self Service Action (Change Address or Reschedule) -->
                <div class="pt-2">
                    <a href="https://wa.me/212773271042?text={{ rawurlencode($whatsappChangeMsg) }}" target="_blank" class="w-full bg-[#25D366] hover:bg-[#20ba59] text-white font-black py-3.5 rounded-2xl text-xs flex items-center justify-center gap-2 transition shadow-sm active:scale-95">
                        <span>💬 تواصل معنا لتعديل العنوان أو وقت الاستلام</span>
                    </a>
                </div>
    ...
    <footer class="text-center text-xs text-slate-400 space-y-2">
        <p>جميع الحقوق محفوظة © {{ date('Y') }} MED EXPRESS</p>
    </footer>
